...
- [x] (2026-07-19 17:00Z) Drafted and architect-reviewed the seven required
  ...
  Dependency-sensitive records remain proposed until their locked graphs pass
  MSRV, license, and platform validation.
...
- [ ] Complete Milestone 1: create the Rust workspace, CI, and conformance-test
  harness.
...
- Decision: accept a monitored cargo-deny exception for RUSTSEC-2024-0436 until
  ...
  Date/author: 2026-07-19, primary agent pending architect confirmation.
...
source, and text-policy checks. Milestone 1 begins with neutral fixtures and a
green conformance-harness self-test before any scientific implementation.
...
package as the public `qslib` facade. The intended members are:

- `qslib-core`: identifiers, scalar and error policy, basis states, geometry,
  weighted interactions, operators, canonical models, symmetry primitives, and
  observable definitions;
- `qslib-exact`: basis enumeration, matrix construction, Hermitian eigensolvers,
  exact thermodynamics, and exact evolution;
- `qslib-variational`: local-energy aggregation, QGT and force statistics,
  TDVP linear solves, diagnostics, and integrators;
- `qslib-sse`: SSE decomposition, state, updates, measurements, deterministic
  parallel chains, and thermal diagnostics;
- `qslib-io`: versioned configuration, manifests, artifacts, checksums, and
  checkpoints;
- `qslib-python`: PyO3 and NumPy bindings for intentionally stable kernels;
- `qslib-cli`: physicist-first commands built only on public library APIs;
...
The release-candidate demonstration must include commands equivalent to:
    ...
    cargo run -p qslib-cli -- model validate examples/heisenberg_disordered_4.yaml
    cargo run -p qslib-cli -- exact ground-state examples/heisenberg_disordered_4.yaml
    cargo run -p qslib-cli -- exact evolve examples/tfim_4.yaml --t-max 0.1
    cargo run -p qslib-cli -- sse run examples/tfim_thermal_4.yaml
